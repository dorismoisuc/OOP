...
private:
	...
	vector<Chore> chores;
	vector<std::unique_ptr<UndoRedo>> undoStack;
	vector<std::unique_ptr<UndoRedo>> redoStack;
	SavedChores* saved_chores;

public:
	//initializes the service by the repository 
	Service(const FileRepo& r, RepoServant& conservant) :repo{ r }, repoServant{ conservant } { this->saved_chores = nullptr; }
